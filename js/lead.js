...
function submitLead(){	 
	...
	if(email==''){
		$('#leadcontent').append('<div class="span12 text-center" id="warning" style="color:#fff;">* Email is required *</div>');
		$('#email').focus();
		return false;
		...
		$('#leadcontent').append('<div class="span12 text-center" id="warning" style="color:#fff;">* Please enter a valid email address *</div>');
		$('#email').focus();
		return false;
	}else{	
		$("#leadform").hide();	
			...
			$.post("http://www.api.contrib.com/forms/saveleads",
				{   domain:domain,
					...
					if(res.success=='false'){
						...
						$('#leadcontent').append('<div class=" text-center" id="response" style="background: none repeat scroll 0% 0% rgb(255, 255, 255); border-radius: 10px; padding: 15px 0px;" >Thanks for showing your interest on '+domain+'!<br><br>Your email will never be sold and kept strictly for notification when we launch!<br><br> Please check your e-mail for further info. Thank You! <br><br>'+
						'<form target="_blank" action="http://www.contrib.com/signup/challenge/challenger?domain='+domain+'" method="post">'+
						'<input type="hidden" id="pemail" name="email" value="'+email+'"/>'+
						'<button class="btn btn-warning">Continue Signup to '+domain+' and Submit Your Winning Entry</button></form><br>'+
						'<div style="padding:10px;width:70%;opacity:.9;margin:0px auto;background:#fafafa;border-radius:8px;color:#000 !important;"><p>To share with your friends, click &ldquo;Share&rdquo; and &ldquo;Tweet&rdquo;:</p>'+
						...
						$('#leadcontent').append('<div class=" text-center" id="response" style="background: none repeat scroll 0% 0% rgb(255, 255, 255); border-radius: 10px; padding: 15px 0px;">'+res.success+'<br><br>Visit our <a href="http://www.contrib.com/brand/details/'+domain+'" target="_blank">'+domain.toUpperCase()+' brand page</a> for further details. <br><br>Thank you! <br><br></div>');
					}
				}
			);
