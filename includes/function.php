...
class DIR_LIB_2 {
		...
		if (!$result) {
			...
			while($row = mysql_fetch_array($result)){
				...
				$output .= '<item>';
					...
					$output .= '<link>'.$domainUrl.$row['URLName'].'-'.$row['ChallengeId'].'.html</link>';
	...
	function GetRSSchallenge($domainUrl,$id,$limit=NULL){
		...
		if (!$result) {
			...
			while($row = mysql_fetch_array($result)){
				//$site = $this->GetTableInfo('Domains','DomainName','DomainId',$id);
				$url = $domainUrl.$row['URLName'].'-'.$row['ChallengeId'].'.html';
				$output .= '<item>';
					$output .= '<title>'.$row['ChallengeTitle'].'</title>';
	...
	function GetRSSuser($domainUrl,$id,$limit=NULL){
	...
	function GetRSSApp($domainUrl,$id,$limit=NULL){
				...
				$html .= '
					<div class="jbox_container">
                            	<div class="jtab_mnu_cont">
                                     <div class="jtab"><a href="'.$row['URLName'].'-'.$challengeid.'.html"><img src="http://mychallenge.com/images/home_ico.png" height="13" width="15"></a></div>  
	...
	function GetAppSingle($appId){
		...
		function ShowChallengeSponsorDetails($challengeid){
			...
			if(!sql){
				...
				$row = mysql_fetch_array($sql);
				$html = '<div>
					<img src="'.$row['Photo'].'" width="270" height="100"><br>
					<p><a href="company-profile-'.$row['Username'].'.html">'.ucfirst($row['Username']).'</a><br>
					'.$this->GetTableInfo('ChallengeMembers','Country','ChallengeMemberId',$companyid).' &mdash; '.$this->GetTableInfo('ChallengeMembers','Email','ChallengeMemberId',$companyid).'<br>
					'.$row['Minibio'].'<br><i><a href="'.$row['Website'].'">'.$row['Website'].'</a></i>
					</p>
					</div>';	
				return $html;
			}
		...
		function GetAppliFiles($appid){
			$result = mysql_query("select * from AppFiles where AppId = '$appid'") or die(mysql_error());
			$array = array();
			while($row = mysql_fetch_array($result)){
				$array[] = $row['FilePath'];
			}
