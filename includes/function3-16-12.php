	...
	function ShowLatestApplications($limit=NULL){
    	...
    	<div class="jgal-img">
        	<div class="jgal-img-hover" style="display:table-cell; vertical-align:bottom;  background-image:url('.$this->GetTableInfo('AppImages','ImagePath','AppId',$row['AppId']).');">
				<div class="jgal-info">
                	<div><a href="application-'.$row['AppId'].'.html">'.$row['AppName'].'</a></div>
                	<div>'.$this->GetTableInfo('ChallengeMembers','UserName','ChallengeMemberId',$row['ChallengeMemberId']).'</div>
                </div>            
...
'; $cnt=$i;
	...
	function GetAppSingle($appId){
		...
		function SendEmail($to,$subject,$message,$username){
			...
			$mailbody.="<tr><td><img src='http://earthchallenge.com/v2/images/logo-earthchallenge2.png' width='602' height='38' alt='&nbsp;&nbsp;EarthChallenge' style='background-color:#1784C9;color:#fff;font:bold 24px Tahoma,Verdana,sans-Serif;display:block'></td></tr>";
			$mailbody.="<tr><td>";
			...
			$mailbody.="<tr class='mtext'><td>EarthChallenge Team</td></tr>";
